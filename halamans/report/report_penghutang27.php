...
$data = mysqli_query($database,"SELECT id_penghutang, nominal, inquiri, date_updated, SUM(nominal) AS total_deposit from bank_hutang WHERE id_penghutang=$id_penghutang GROUP BY id_penghutang");
   while($last_d = mysqli_fetch_array($data)){
...
$data1 = mysqli_query($database,"SELECT id_penghutang, nominal, inquiri, date_updated, SUM(nominal) AS total_withdraw from bank_hutang WHERE id_penghutang =$id_penghutang GROUP BY id_penghutang");
   while($last_d1 = mysqli_fetch_array($data1)){
    ...
    ?>
            ...
            <tr>
              <td><?php echo $no++ ?></td>
               <td><?php echo $last_d['id_penghutang']; ?></td>
        <!--        <td><?php//s echo @rupiah($d['nominal']); ?></td> -->
               <td><?php echo @rupiah($last_d['total_deposit']); ?></td>
            <td><?php echo @rupiah($last_d1['total_withdraw']); ?></td> 
            <td><?php echo @rupiah($last_d['total_deposit']+$last_d1['total_withdraw']); ?></td>
               <td><?php echo $last_d['date_updated']; ?></td>
                </tr>
