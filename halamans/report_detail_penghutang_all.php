...
$jenis_kas=0;
$sqlTampil1= "SELECT * from bank_hutang WHERE inquiri='deposit'";
...
<?php if ($tampil1->num_rows > 0) {
  ...
  while($row1 = $tampil1->fetch_array()){ ?>
    ...
    $jenis_kas=0;
$sqlTampil2= "SELECT * from bank_hutang WHERE inquiri='withdraw'";
$tampil2 = $database->query($sqlTampil2);
?>
<?php if ($tampil2->num_rows > 0) {
  // jika data benar
  while($row12= $tampil2->fetch_array()){ ?>
              ...
              <td><?php echo @rupiah($row2['nominal']); ?></td>
               <td><?php echo @rupiah($row1['date_updated']); ?></td>
              </tr>
            <tr>
                <td>ZGRAND TOTAL</td>
