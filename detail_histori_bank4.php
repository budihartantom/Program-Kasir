...
$sqlTampil= "SELECT bank_akun.id, bank_akun.rekening, bank_akun.nominal1, bank_akun.nominal, bank_akun.inquiri, bank_akun.date_updated, bank_account.account_name, bank_account.account_number from bank_akun, bank_account WHERE bank_akun.rekening=bank_account.rekening ORDER BY rekening";
$tampil = $database->query($sqlTampil);
?>
<?php if ($tampil->num_rows > 0) {
  // jika data benar
  while($d = $tampil->fetch_assoc()){ ?>
